            ...
            if any("cdn" in s for s in dns_record_data):
                ...
                try:
                    response = requests.get(
                        'https://api.ultradns.com/zones/' + domain + '/rrsets/All/' + domain, headers=headers)
...
def FlushChinaOnly(domainlist):
    ...
    Collection = db["data"]
    cdn1IP = socket.gethostbyname('cdn1.dwdns3.services.')  # non-https
    cdn2IP = socket.gethostbyname('cdn2.dwdns3.services.')  # non-https
    cdn3IP = socket.gethostbyname('cdn3.dwdns3.services.')  # non-https, default
    cdn5IP = socket.gethostbyname('cdn5.dwdns3.services.')  # non-https
    cdn6IP = socket.gethostbyname('cdn6.dwdns3.services.')  # non-https
    ...
    for domain in domainlist:
        try:
            ...
            if any("cdn" in s for s in dns_record_data):
                ...
                if(str(dns_record_data[www_CN_sequence]) == "cdn1.dwdns3.services."):
                    cdn_value = "cdn1"
                ...
                try:
                    ...
                    for i in range(len(DNSJSON['rrSets'][0]['profile']['rdataInfo'])):
                        ...
                        if str(DNSJSON['rrSets'][0]['profile']['rdataInfo'][i]['geoInfo']).find("\'CN\'") != -1:
                            CN_sequence = i
                            patch_jsonbody = DNSJSON['rrSets'][0]
                            if cdn_value == "cdn1":
                                patch_jsonbody['rdata'][CN_sequence] = cdn1IP
                            if cdn_value == "cdn2":
                                patch_jsonbody['rdata'][CN_sequence] = cdn2IP    
                            if cdn_value == "cdn3":
                                patch_jsonbody['rdata'][CN_sequence] = cdn3IP
                            if cdn_value == "cdn5":
                                patch_jsonbody['rdata'][CN_sequence] = cdn5IP
                            if cdn_value == "cdn6":
                                patch_jsonbody['rdata'][CN_sequence] = cdn6IP    
                            ...
                            try:
                                response_put = requests.put(
                                    'https://api.ultradns.com/zones/' + domain + '/rrsets/A/' + domain, headers=headers, json=patch_jsonbody)
                                ...
                                if response.status_code == 200:
                                    print(
                                        "\nDomain: " + domain + " successfully updated China only IP records\n")
                                else:
                                    print(
                                        "\nDomain: " + domain + " failed to change China only IP records\n")
