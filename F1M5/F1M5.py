...
# 讀取第一個Excel文件
shift = input("Please input which shift(M or S):")
game_test = input("Please input game test filename: ")
web_test = input("Please input web test filename: ")
...
for i in range(13, 78):
    try:
        value1_City_ISP = sheet_game.cell(
            row=i, column=2).value + '_' + sheet_game.cell(row=i, column=3).value
        value1_StepName = sheet_game.cell(row=i, column=4).value

        if value1_City_ISP in city_isp_map and value1_StepName in step_map:
            sheet2.cell(row=step_map[value1_StepName], column=city_isp_map[value1_City_ISP]).value = sheet_game.cell(
                row=i, column=6).value
        else:
...
for c in range(16, 56):
    try:
        value2_City_ISP = sheet_web.cell(
            row=c, column=2).value + '_' + sheet_web.cell(row=c, column=3).value
        ...
        if value2_City_ISP in city_isp_map_web and value2_StepName in test_map:
            sheet3.cell(row=test_map[value2_StepName], column=city_isp_map_web[value2_City_ISP]).value = sheet_web.cell(
                row=c, column=7).value
        else:
