...
class zabbixProfile(object):
    ...
    def update_host_template(self, hostID, templates):
        jsonbody={
            "jsonrpc":"2.0",
            "method":"host.update",
            "params":{
...
class massupdate_template(zabbixProfile):
    def __init__(self):
        super().__init__()
        super().inputPassword()
        logging.info('[{date}][INFO] Trying to get group list.'.format(date=cts()))
    ...
    def massupdate_template_change(self):
        self.templates_menu()
        get_profile_number = input('Type number for which template you want to apply:')
        try:
            selected_template=self.templates_json['profile'][int(get_profile_number)-1]
        ...
        for item in domain_input:
            domain=item.split(':')
            logging.info('[{date}][INFO] update host for \'{domain_name}\' with {template_name}..'.format(date=cts(), domain_name=item, template_name=template_name))
            print('[{date}][INFO] Update host for \'{domain_name}\' with {template_name}..'.format(date=cts(), domain_name=item, template_name=template_name))
